...
void UniversalHwid(void) {
    ...
    const char* mid_paths[] = {"/etc/machine-id", "/var/lib/dbus/machine-id", 0};
    for (const char** mp = mid_paths; *mp; mp++) {
        if ((h = os_open_file(*mp))) {
            n = os_read_file(h, curr, 33); os_close_file(h);
            while (n-- > 0 && *curr > 32 && curr < end) curr++;
            break; } }
    if ((h = os_open_file("/proc/partitions"))) {
        unsigned char* p = curr; 
        int rd = os_read_file(h, p, 1024); os_close_file(h);
        unsigned char* stop = p + rd - 5;
        while (p < stop) {
            if ((*p == 's' && *(p+1) == 'd') || (*p == 'n' && *(p+1) == 'v' && *(p+2) == 'm')) {
                unsigned char* start = p;
                while (*p > 32) p++;
                unsigned char tmp = *p; *p = '\0';
                os_snprintf((char*)(end - 128), 128, "/sys/class/block/%s/device/serial", start);
                h2 = os_open_file((char*)(end - 128)); *p = tmp;
                if (h2) {
                    n = os_read_file(h2, curr, 64); os_close_file(h2);
                    while (n-- > 0 && *curr > 32 && curr < end) curr++;
                    break; } }
            p++; } }
    if ((h = os_open_file("/proc/net/dev"))) {
        unsigned char* p = curr;
        int rd = os_read_file(h, p, 1024); os_close_file(h);
        unsigned char* stop = p + rd - 10;
        while (p < stop) {
            if (*p == ':' && *(p-1) != ' ' && *(p-1) != 'o') {
                unsigned char* start = p;
                while (start > GlobalBuf && *(start-1) != ' ' && *(start-1) != '\n') start--;
                unsigned char tmp = *p; *p = '\0';
                os_snprintf((char*)(end-128), 128, "/sys/class/net/%s/address", start);
                h2 = os_open_file((char*)(end-128)); 
                *p = tmp;
                if (h2) {
                    n = os_read_file(h2, (unsigned char*)(end-128), 32); os_close_file(h2);
                    unsigned char* m = (unsigned char*)(end-128);
                    while (n-- > 0 && *m > 32 && curr < end) { 
                        if ((*m >= '0' && *m <= '9') || (*m >= 'a' && *m <= 'z') || (*m >= 'A' && *m <= 'Z')) *curr++ = *m;
                        m++; }
                    break; } }
            p++; } }
    if ((h = os_open_file("/proc/sys/kernel/osrelease"))) {
        n = os_read_file(h, (unsigned char*)(end-128), 64); os_close_file(h);
        unsigned char* v = (unsigned char*)(end-128);
        while (n-- > 0 && *v > 32 && curr < end) {
            if ((*v >= '0' && *v <= '9') || (*v >= 'a' && *v <= 'z') || (*v >= 'A' && *v <= 'Z')) *curr++ = *v;
            v++; } }
    *curr = '\0';
