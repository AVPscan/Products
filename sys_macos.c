...
void delay_ms(int ms) {
    ...
    struct timespec check_start; clock_gettime(CLOCK_MONOTONIC, &check_start); uint32_t safety = 0;
    while ((get_cycles() - start_time) < total_cycles) { // __asm__ volatile("pause");
        if (++safety > 2000) { struct timespec now; clock_gettime(CLOCK_MONOTONIC, &now);
